...
mod platform_tests {
    ...
    mod linux {
        ...
        use promptbridge::platform::{
            PlatformClipboard, PlatformNotifier, PlatformShortcutInstaller,
        };
        ...
        fn test_linux_clipboard_set_text() {
            ...
            let result = std::process::Command::new("which").arg("xclip").output();
        ...
        fn test_linux_shortcut_install_config_creation() {
            ...
            // Clean up after test
            if let Ok(_) = result {
                let config_dir = dirs::config_dir().map(|d| d.join("promptbridge"));
                ...
                let bin_dir = dirs::home_dir().map(|d| d.join(".local/bin"));
    ...
    mod macos {
        ...
        fn test_macos_shortcut_install_config_creation() {
            ...
            if let Ok(_) = result {
                let config_dir =
                    dirs::home_dir().map(|d| d.join("Library/Application Support/promptbridge"));
