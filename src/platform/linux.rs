...
impl PlatformDialog for LinuxPlatform {
    ...
    fn show_text_info(&self, title: &str, text: &str) -> Result<TextInfoResult> {
        ...
        let status = result
            .wait()
            .map_err(|e| PromptBridgeError::Engine(format!("Failed to wait for dialog: {}", e)))?;
...
impl ProgressDialogHandle for LinuxProgressDialogHandle {
    fn close(&self) -> Result<()> {
        Command::new("kill")
            ...
            .status()
            .map_err(|e| {
                PromptBridgeError::Engine(format!("Failed to close progress dialog: {}", e))
            })?;
        Ok(())
    }
}
...
impl PlatformNotifier for LinuxPlatform {
    fn show_notification(&self, title: &str, message: &str) -> Result<()> {
        Command::new("notify-send")
            .args(&["--app-name=PromptBridge", "--icon=info", title, message])
            .status()
            .map_err(|e| {
...
impl PlatformClipboard for LinuxPlatform {
    ...
    fn set_text(&self, text: &str) -> Result<()> {
        ...
        if let Some(ref mut stdin) = child.stdin {
            stdin.write_all(text.as_bytes()).map_err(|e| {
                PromptBridgeError::Clipboard(format!("Failed to write to clipboard: {}", e))
            })?;
        }

        // Wait for xclip to finish processing
        child.wait().map_err(|e| {
            PromptBridgeError::Clipboard(format!("Failed to wait for clipboard: {}", e))
        })?;
