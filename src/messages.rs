...
pub const MSG_NO_COMMAND_SPECIFIED: &str =
    "No target CLI command specified. Example: `promptbridge exec -- claude`";
pub const MSG_INPUT_PROMPT_EMPTY: &str =
    "Input prompt is empty. Provide text as argument or pipe via stdin.";
...
pub const TIP_EXEC_ERROR: &str =
    "Ensure the target CLI binary is installed and executable in your PATH.";
...
pub fn format_exec_argv_info(transformed_prompt: &str) -> String {
    format!(
        "[PromptBridge] Transformed argv prompt -> \"{}\"",
        transformed_prompt
    )
}
...
pub fn format_provider_list_item(
    name: &str,
    is_default: bool,
    provider_type: &str,
    model: &str,
    ...
    format!(
        " - {}{}: type={}, model={}",
        name, default_marker, provider_type, model
    )
}
